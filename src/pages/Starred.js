...
function Starred() {
  const { state } = useMail();
  const allStarredMails = state?.allMails?.filter((mail) => mail.starred);
      ...
      <div className="grid grid-cols-8 overflow-hidden lg:max-w-6xl px-5 gap-2 grow">
        ...
        <div className="mainContainerGrid">
          <div className="flex flex-row justify-between py-2">
            <h2 className="font-bold text-2xl ml-4 pt-4">Starred</h2>
            <MdFilterList className="mr-1 w-10 h-10" />
          </div>
          <hr className="border dark:border-white  border-black" />
          <div>
            {allStarredMails.length > 0 ? (
              allStarredMails?.map((mail) => (
                <div key={mail.id}>
                  <MailCard mail={mail} />
